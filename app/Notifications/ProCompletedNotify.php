...
class ProCompletedNotify extends Notification 
{
    use Queueable;
    public $cususer;
    public $prouser;
    ...
    public function __construct($cususer, $prouser, $bid, $project_call)
    {
        ...
        $this->project_subject = ' Project Marked as Completed. ';
    }
    ...
    {
        $client = new Client();
        $res = $client->request('POST', 'https://account.kudisms.net/api/?username=hello@ubuy.ng&password=ubuy&message=Hello, '.$this->project_call->cus_name.' just released your funds for '.$this->project_call->sub_category_name.' task. login to https://ubuy.ng/dashboard/ for details &sender=Ubuy.ng&mobiles='.$this->prouser->number.'', [
        ]);
        ...
        // $url = url('/dashboard/bids/'.$project_id);
                    ...
                    ->view(
                        'emails.pros.notify_completed_email',
